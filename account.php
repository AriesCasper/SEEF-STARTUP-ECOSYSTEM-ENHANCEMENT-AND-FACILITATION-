...
EOF;	
				    ...
				    $sql = "SELECT * FROM `user_connections` WHERE (`sender_user_id` = '$main_user' OR `receiver_user_id` = '$main_user') AND `connection_status` = 'accepted'";
				    $res = $db->query($sql) or die($db->error);
				    while ($row = $res->fetch_array()) {
				        if ($row['sender_user_id'] != $main_user) {
				    ...
				    // Get Data of Stranger User
				    $sql_Stranger_User = "SELECT * FROM `users` WHERE `user_id` = '$stranger_user_id'";
				    $res_Stranger_User = $db->query($sql_Stranger_User) or die($db->error);
				    $row_Stranger_User = $res_Stranger_User->fetch_array();

				    $Stranger_User_Fullname = $row_Stranger_User['fullname'];
				    $Stranger_User_Age = $row_Stranger_User['age'];
				    $Stranger_User_Gender = $row_Stranger_User['gender'];
				    $Stranger_User_About = $row_Stranger_User['about'];
				    $Stranger_User_Skills = $row_Stranger_User['skills'];
				    $Stranger_User_Projects = $row_Stranger_User['projects'];
				    $Stranger_User_Country = $row_Stranger_User['address_country'];

				    // Find all Connections of the stranger user
				    $Stranger_User_Connections = [];
				    $sql = "SELECT * FROM `user_connections` WHERE (`sender_user_id` = '$stranger_user_id' OR `receiver_user_id` = '$stranger_user_id') AND `connection_status` = 'accepted'";
				    $res = $db->query($sql) or die($db->error);
				    while ($row = $res->fetch_array()) {
				        if ($row['sender_user_id'] != $stranger_user_id) {
				            $Stranger_User_Connections[] = $row['sender_user_id'];
				        } else {
				            $Stranger_User_Connections[] = $row['receiver_user_id'];
				        }
				    }
				    ...
				    $age_diff = abs($Main_User_Age - $Stranger_User_Age);
				    $score += (100 - $age_diff);

				    // Gender Factor (Arbitrarily prefer different genders)
				    if ($Main_User_Gender != $Stranger_User_Gender) {
				        $score += 50;
				    } else {
				        $score += 20;
				    }
				    ...
				    // Country Factor
				    if ($Main_User_Country == $Stranger_User_Country) {
				        $score += 30;
				    }

				    // Skills Overlap Factor
				    $main_user_skills = explode(',', $Main_User_Skills);
				    $stranger_user_skills = explode(',', $Stranger_User_Skills);
				    $common_skills = array_intersect($main_user_skills, $stranger_user_skills);
				    $score += count($common_skills) * 10;

				    // Projects Overlap Factor
				    $main_user_projects = explode(',', $Main_User_Projects);
				    $stranger_user_projects = explode(',', $Stranger_User_Projects);
				    $common_projects = array_intersect($main_user_projects, $stranger_user_projects);
				    $score += count($common_projects) * 5;

				    // Connections Overlap Factor
				    $common_connections = array_intersect($Main_User_Connections, $Stranger_User_Connections);
				    $score += count($common_connections) * 15;

				    // About Similarity Factor (based on length similarity)
				    $about_length_diff = abs(strlen($Main_User_About) - strlen($Stranger_User_About));
				    $score += (50 - $about_length_diff);

				    // Random Arbitrary Adjustments
				    if (strpos($Stranger_User_About, $Main_User_Fullname) !== false) {
				        $score += 25;
				    }
				    if (strpos($Main_User_About, $Stranger_User_Fullname) !== false) {
				        $score += 25;
				    }
				    if (strlen($Stranger_User_Fullname) > 10) {
				        $score += 10;
				    }
				    if (strlen($Main_User_Fullname) > 10) {
				        $score += 10;
				    }

				    //  Threshold for Match
    $is_good_match = ($score > 150) ? true : false;
